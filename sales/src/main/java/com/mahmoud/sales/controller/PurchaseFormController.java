    ...
    public void onSavePurchase() {
        try {
            ...
            List<Purchasedetail> detailsToSave = purchasedetailList.stream()
                    .filter(this::isRowFilled)
                    .map(oldDetail -> {       //انا
                        // Create a NEW detail object without ID (critical fix!)
                        Purchasedetail freshDetail = new Purchasedetail();
                        freshDetail.setItem(oldDetail.getItem());
                        freshDetail.setQuantity(oldDetail.getQuantity());
                        freshDetail.setPurchasingPrice(oldDetail.getPurchasingPrice());
                        freshDetail.setComulativePrice(oldDetail.getComulativePrice());
                        // DO NOT copy the ID - let the service assign it
                        return freshDetail;
                    })
                    .collect(Collectors.toList());
